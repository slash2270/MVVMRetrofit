...
class DataModel {
    ...
    fun dynamicData(
        ...
        val  retrofit = Retrofit.Builder()
            ...
            .addConverterFactory(GsonConverterFactory.create(gson)) // Or .addCallAdapterFactory(RxJava3CallAdapterFactory.createWithScheduler(Schedulers.io()))
        ...
        observable1.subscribeOn(Schedulers.io()) // （初始被观察者）切换到IO线程进行网络请求1
            ...
            .subscribe({ response -> // 对第2次网络请求返回的结果进行操作
                response(response, list)
                adapter(list, context, binding) //資料合併1
            }, { println("網路連線失敗") })
    ...
    private fun adapter(list: ArrayList<MainBean>, context: Context, binding: ActivityMainBinding) {
